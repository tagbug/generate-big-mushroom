...
- **本地排行榜系统**:
    - 自动保存每次游戏的分数到本地存储（localStorage）。
    - 显示前10名的最高分数记录，包括分数、使用的皮肤和游戏时间。
    - 创下新纪录时提供特殊的视觉效果和音效反馈。
    - 支持按皮肤分类的个人最佳成绩查看。
    - 数据持久化存储，浏览器重启后记录依然保留。
...
-   `src/components/`: 存放所有 React 组件。
    ...
    -   `Leaderboard.tsx`: 排行榜组件，显示本地最高分数记录。
...
-   `src/hooks/`: 存放自定义 React Hooks
    ...
    -   `useI18nSkin.ts`: 皮肤相关的国际化 Hook，提供皮肤名称和物体名称的本地化支持。
-   `src/utils/`: 存放工具函数
    -   `leaderboard.ts`: 排行榜管理工具，处理分数保存、排序和本地存储操作。
    -   `audioManager.ts`: 音效管理工具，提供游戏音效播放功能。
